...
Follow these steps to use the UI to create a dataset from existing traces. For reference information, see  [MLflow evaluation dataset UI](/aws/en/mlflow3/genai/eval-monitor/concepts/eval-datasets#ui).
...
Follow these steps to use the SDK to create a dataset. For reference information, see [Evaluation dataset reference](/aws/en/mlflow3/genai/eval-monitor/concepts/eval-datasets).
...
After collecting expert feedback, you can align judges to match the human feedback. See [Align LLM judges with human feedback](/aws/en/mlflow3/genai/eval-monitor/align-judges).
...
Generating synthetic data can expand your testing efforts by quickly creating diverse inputs and covering edge cases. See [Synthesize evaluation sets](/aws/en/agents/agent-evaluation/synthesize-evaluation-set).
...
* Evaluation datasets cannot be stored in catalogs encrypted with [customer-managed keys (CMK)](/aws/en/security/keys/cmek-unity-catalog). Workspaces with CMK are supported as long as the dataset is stored in a non-CMK catalog.
